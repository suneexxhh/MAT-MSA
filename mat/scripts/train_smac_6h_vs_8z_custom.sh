#!/bin/sh
# 6h_vs_8z 地图专用训练脚本，支持自定义 n_levels、mid_chunk_size 和 seed
...
#   bash train_smac_6h_vs_8z_custom.sh 0
#   参数说明：$1=gpu_id
#   实验名称：mat_msa_6h_vs_8z_adaptive_seed1
#   日志文件：logs/mat_msa_6h_vs_8z_adaptive_seed1.log
#
# 方法2（使用地图自适应配置，指定 seed）：
#   bash train_smac_6h_vs_8z_custom.sh 0 42
#   参数说明：$1=gpu_id, $2=seed
#   实验名称：mat_msa_6h_vs_8z_adaptive_seed42
#   日志文件：logs/mat_msa_6h_vs_8z_adaptive_seed42.log
#
# 方法3（手动指定 n_levels 和 mid_chunk_size，默认 seed=1）：
#   bash train_smac_6h_vs_8z_custom.sh 0 3 4
#   参数说明：$1=gpu_id, $2=n_levels, $3=mid_chunk_size
#   实验名称：mat_msa_6h_vs_8z_L3_c4_seed1
#   日志文件：logs/mat_msa_6h_vs_8z_L3_c4_seed1.log
#
# 方法4（手动指定 n_levels、mid_chunk_size 和 seed）：
#   bash train_smac_6h_vs_8z_custom.sh 0 3 4 42
#   参数说明：$1=gpu_id, $2=n_levels, $3=mid_chunk_size, $4=seed
#   实验名称：mat_msa_6h_vs_8z_L3_c4_seed42
#   日志文件：logs/mat_msa_6h_vs_8z_L3_c4_seed42.log

env="StarCraft2"
map="6h_vs_8z"  # 固定为 6h_vs_8z 地图
gpu_id="$1"
algo="mat"
...
if [ -n "$2" ] && [ -z "$3" ] && [ "$2" -eq "$2" ] 2>/dev/null; then
    ...
    n_levels=""
    mid_chunk_size=""
    exp="mat_msa_6h_vs_8z_adaptive_seed${seed}"
    echo "=================================="
    ...
    n_levels="$2"
    mid_chunk_size="$3"
    exp="mat_msa_6h_vs_8z_L${n_levels}_c${mid_chunk_size}_seed${seed}"
    echo "=================================="
