...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        #     from mat.algorithms.mat.algorithm.ma_transformer import MultiAgentModel as MAT
        ...
        # ========== 打印参数传递信息 ==========
        map_name_param = getattr(args, 'map_name', None)
        mid_chunk_param = getattr(args, 'cms_mid_chunk_size', 4)
        n_levels_param = getattr(args, 'n_cms_levels', 3)
        use_map_adaptive_param = getattr(args, 'use_map_adaptive', True)

        print("\n" + "="*80)
        print("[1/5] TransformerPolicy: Creating MultiAgentTransformer")
        print("="*80)
        print(f"  map_name:          {map_name_param}")
        print(f"  mid_chunk_size:    {mid_chunk_param}")
        print(f"  n_levels:          {n_levels_param}")
        print(f"  num_agents:        {num_agents}")
        print(f"  use_map_adaptive:  {use_map_adaptive_param}")
        print("="*80 + "\n")
        ...
        self.transformer = MultiAgentTransformer(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                               ...
                               mid_chunk_size=mid_chunk_param,
                               map_name=map_name_param,
                               n_levels=n_levels_param,
                               use_map_adaptive=use_map_adaptive_param)
        # self.transformer = MultiAgentModel(
        ...
        #     n_cms_levels=getattr(args, 'n_cms_levels', 3),  # ← 新增
        #     cms_dropout=getattr(args, 'cms_dropout', 0.0),  # ← 新增
        # )
