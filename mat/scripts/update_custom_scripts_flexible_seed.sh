#!/bin/bash
# 批量更新所有 custom 脚本，优化 seed 参数使用方式
...
#   bash train_smac_MAP_NAME_custom.sh 0 1 "" "" 42
#   参数说明：$1=gpu_id, $2=seed
#   注意：$2 必须是数字才会被识别为 seed
#
...
#   参数说明：$1=gpu_id, $2=n_levels, $3=mid_chunk_size, $4=exp_name(可选), $5=seed
#
# 方法5（手动指定所有参数）：
#   bash train_smac_MAP_NAME_custom.sh 0 3 4 my_experiment 42
#   参数说明：$1=gpu_id, $2=n_levels, $3=mid_chunk_size, $4=exp_name, $5=seed
...
if [ -n "$2" ] && [ -z "$3" ] && [ "$2" -eq "$2" ] 2>/dev/null; then
    ...
    n_levels=""
    mid_chunk_size=""
    exp="${4:-map_adaptive_MAP_NAME_seed${seed}}"
    echo "=================================="
    ...
    # 方法3/4/5：手动指定 n_levels 和 mid_chunk_size
    seed="${5:-1}"
    use_map_adaptive="false"
    n_levels="$2"
    mid_chunk_size="$3"
    exp="${4:-MAP_NAME_manual_${n_levels}levels_c${mid_chunk_size}_seed${seed}}"
    echo "=================================="
...
EOF
    ...
    echo "Updated: train_smac_${map}_custom.sh (flexible seed support)"
done

echo ""
echo "All custom scripts updated with flexible seed support!"
