...
#!/bin/sh
# ${map} 地图专用训练脚本，支持自定义 n_levels、mid_chunk_size 和 seed
#
# 使用方法：
# 方法1（使用地图自适应配置）：
...
#
# 方法4（手动指定参数 + 实验名称 + seed）：
#   bash train_smac_${map}_custom.sh 0 3 4 my_experiment 42
...
if [ -z "\$2" ] || [ -z "\$3" ]; then
    ...
    n_levels=""
    mid_chunk_size=""
    exp="\${4:-map_adaptive_${map}_seed\${seed}}"
    echo "=================================="
    ...
    n_levels="\$2"
    mid_chunk_size="\$3"
    exp="\${4:-${map}_manual_\${n_levels}levels_c\${mid_chunk_size}_seed\${seed}}"
    echo "=================================="
...
EOF
    ...
    echo "Updated: train_smac_${map}_custom.sh (with seed support)"
done

echo ""
echo "All custom scripts updated with seed support!"
