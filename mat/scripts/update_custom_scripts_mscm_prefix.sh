#!/bin/bash
# 批量更新所有 custom 脚本，将实验名称前缀改为 mscm
...
if [ -n "$2" ] && [ -z "$3" ] && [ "$2" -eq "$2" ] 2>/dev/null; then
    ...
    n_levels=""
    mid_chunk_size=""
    exp="${4:-mscm_MAP_NAME_adaptive_seed${seed}}"
    echo "=================================="
    ...
    n_levels="$2"
    mid_chunk_size="$3"
    exp="${4:-mscm_MAP_NAME_L${n_levels}_c${mid_chunk_size}_seed${seed}}"
    echo "=================================="
...
EOF
    ...
    echo "Updated: train_smac_${map}_custom.sh (mscm prefix)"
done

echo ""
echo "All custom scripts updated with mscm prefix!"
